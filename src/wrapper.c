...
int w(double* bc_, double* v_, double* sh_, double* angle_, double* zero_, double* windspeed_, double* windangle_) {
  ...
  */
  ...
  double zeroangle = zero_angle(G1, *bc_, *v_, 1.6, *zero_, 0);
  ...
  k = Ballistics_solve(&solution, G1, *bc_, *v_, *sh_, *angle_, zeroangle, *windspeed_, *windangle_);

  // Now print a simple chart of X / Y trajectory spaced at 10yd increments
  int s=0;
  for (s=0; s<=1000; s+=100) {
    printf("X: %.0f\t\tY: %.2f\t\t\t\tZ: %.2f\n", Ballistics_get_range(solution, s), Ballistics_get_path(solution, s), Ballistics_get_windage(solution, s));
  }
